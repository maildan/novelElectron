...
export const registerKeyboardListener = async () => {
  try {
    ...
    if (!status.isInitialized) {
      Logger.info("Console", 'KeyboardEngine 초기화 중...');
      await keyboardEngine.initialize();
    }
    
    Logger.info("Console", 'KeyboardEngine 리스닝 시작...');
    return keyboardEngine.startListening();
  } catch (error) {
    Logger.error("Console", '키보드 리스너 등록 실패:', error);
    throw error;
  }
};
...
export const getMonitoringStatus = () => {
  try {
    ...
    Logger.error("Console", '모니터링 상태 확인 실패:', error);
    return {
      isActive: false,
      isInitialized: false,
      message: '상태 확인 실패'
