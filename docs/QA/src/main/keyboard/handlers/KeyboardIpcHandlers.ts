...
const SUCCESS_MESSAGES = {
  ...
  HANDLERS_UNREGISTERED: '🔌 키보드 엔진 IPC 핸들러 해제 완료'
};
...
export class KeyboardIpcHandlers {
  ...
  public registerHandlers(handlers: IpcHandler): void {
    ...
    Logger.info('KeyboardIpcHandlers registered handlers', SUCCESS_MESSAGES.HANDLERS_REGISTERED);
  }
  ...
  public registerHandler(channel: string, handler: (...args: unknown[]) => unknown): void {
    ...
    Logger.debug('KeyboardIpcHandlers register handler', `IPC 핸들러 등록: ${channel}`);
  }
  ...
  public unregisterHandlers(): void {
    ...
    Logger.info('KeyboardIpcHandlers unregistered handlers', '🔌 키보드 엔진 IPC 핸들러 해제 완료');
  ...
  public unregisterHandler(channel: string): void {
    ...
    Logger.debug('KeyboardIpcHandlers unregister handler', `IPC 핸들러 해제: ${channel}`);
  }
